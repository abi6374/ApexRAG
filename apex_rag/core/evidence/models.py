...
class EvidencePacket(BaseModel):
    ...
    retrieval_reason: str = Field(..., description="Why this node was selected (from the Planner/Navigator)")
    ...
    graph_relationships: list[dict[str, str]] = Field(default_factory=list, description="Relevant SRG edges")

    confidence_score: float = Field(..., description="Mathematical score of retrieval confidence")
    provenance_chain: list[str] = Field(default_factory=list, description="Trace of agents involved")
    contradiction_flags: list[str] = Field(default_factory=list, description="Detected conflicts with other packets")
