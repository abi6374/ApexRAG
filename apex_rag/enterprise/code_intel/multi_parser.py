...
class MultiLanguageCodeParser(DocumentParser):
    ...
    def _parse_python(self, code: str, root: ASTNode, file_path: str) -> None:
        ...
        for match in func_pattern.finditer(code):
            node = ASTNode(
                ...
                metadata=ASTNodeMetadata(source_file=file_path, custom={"lineno": code.count("\n", 0, match.start()) + 1}),
            )
            root.add_child(node)
        ...
        for match in class_pattern.finditer(code):
            ...
            node = ASTNode(
                ...
                metadata=ASTNodeMetadata(
                    ...
                    custom={"lineno": code.count("\n", 0, match.start()) + 1, "inherits_from": base},
                ),
            )
            root.add_child(node)
    ...
    def _parse_js_ts(self, code: str, root: ASTNode, file_path: str) -> None:
        func_pattern = re.compile(r"(?:function\s+(?P<name>\w+)|const\s+(?P<name2>\w+)\s*=\s*\(.*?\)\s*=>)", re.MULTILINE)
        class_pattern = re.compile(r"^\s*class\s+(?P<name>\w+)(?:\s+extends\s+(?P<base>\w+))?", re.MULTILINE)
    ...
    def _parse_java(self, code: str, root: ASTNode, file_path: str) -> None:
        class_pattern = re.compile(r"(?:public|private|protected)?\s*class\s+(?P<name>\w+)(?:\s+extends\s+(?P<base>\w+))?", re.MULTILINE)
        method_pattern = re.compile(r"(?:public|private|protected|static|\s)+\s+(?P<ret>\w+)\s+(?P<name>\w+)\s*\(.*?\)\s*\{", re.MULTILINE)
    ...
    def _parse_cpp(self, code: str, root: ASTNode, file_path: str) -> None:
        class_pattern = re.compile(r"^\s*class\s+(?P<name>\w+)(?:\s*:\s*(?:public|private|protected)?\s*(?P<base>\w+))?", re.MULTILINE)
        func_pattern = re.compile(r"^\s*(?P<ret>[\w::]+)\s+(?P<name>\w+)\s*\(.*?\)\s*(?:const)?\s*\{", re.MULTILINE)
        ...
        for i, child in enumerate(children):
            if isinstance(child, ASTNode):
                ...
                if inherits:
                    ...
                    base_node = next((c for c in children if isinstance(c, ASTNode) and c.content == inherits), None)
                    if base_node:
                        edges.append(
                            GraphEdge(
