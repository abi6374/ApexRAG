...
class TestRoleBasedAccessControl:
    ...
    async def test_rbac_check_permissions(self, storage: ApexStorage) -> None:
        ...
        ctx_admin = TenantContext(
            tenant_id="tenant_a", user_id="admin-1", roles=[Roles.TENANT_ADMIN]
        )
    ...
    async def test_field_level_security_masking(self, storage: ApexStorage) -> None:
        ...
        assert (
            "revenue = [REDACTED]" in masked.lower()
            or "revenue = [redacted]" in masked.lower()
            or "[redacted]" in masked.lower()
        )
        assert (
            "profit margin = [REDACTED]" in masked.lower()
            or "profit margin = [redacted]" in masked.lower()
            or "[redacted]" in masked.lower()
        )
    ...
    async def test_audit_trail_logging(self, storage: ApexStorage) -> None:
        ...
        # Query audit logs
        logs = await storage.get_audit_logs(tenant_context="tenant_a")
        assert len(logs) == 1
    ...
    async def test_orchestrator_pre_retrieval_rbac(self) -> None:
        ...
        planner = MagicMock()
        navigator = MagicMock()
        navigator._storage = AsyncMock()  # AsyncMock so save_audit_log can be awaited
        critic = MagicMock()

        orchestrator = Orchestrator(planner=planner, navigator=navigator, critic=critic)
    ...
    async def test_custom_rbac_rules_and_evaluations(self, storage: ApexStorage) -> None:
        ...
        await agent.assign_field_permission(
            "CustomRole", "SecretNode", "classified_info", is_allowed=True
        )
    ...
    async def test_dynamic_custom_rules_and_script_evaluators(self, storage: ApexStorage) -> None:
        ...
        TenantContext(tenant_id="tenant_b", user_id="user-2", roles=["Analyst"])
        ...
        assert (
            PolicyEvaluator.evaluate_condition(
        ...
        assert (
            PolicyEvaluator.evaluate_rule(
                PolicyRule(field="name", operator="STARTS_WITH", value="John"), {"name": "John Doe"}
            )
            is True
        )
        assert (
            PolicyEvaluator.evaluate_rule(
                PolicyRule(field="name", operator="ENDS_WITH", value="Doe"), {"name": "John Doe"}
            )
            is True
        )
        assert (
            PolicyEvaluator.evaluate_rule(
                PolicyRule(field="score", operator="GT", value=50), {"score": 75}
            )
            is True
        )
        assert (
            PolicyEvaluator.evaluate_rule(
                PolicyRule(field="score", operator="LTE", value=50), {"score": 30}
            )
            is True
        )
        assert (
