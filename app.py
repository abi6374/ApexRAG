...
import os
import gradio as gr
from apex_rag import ApexIndex, OpenAIProvider, GroqProvider, OllamaProvider
...
async def get_index():
    ...
    if index_instance is None:
        ...
        index_instance = await ApexIndex.create(
            db_url=DB_URL,
            model=provider
        )
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    gr.Markdown("### Structural AI Retrieval — Reading documents like a human, not a bag of chunks.")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            question_input = gr.Textbox(label="Ask a question about the document", placeholder="e.g., Compare the revenue growth between Q2 and Q3.")
    ...
    ingest_btn.click(
        handle_ingest,
        inputs=[file_input],
        outputs=[doc_id_state, ingest_status]
    )
