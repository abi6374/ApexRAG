...
async def run_one_baseline(baseline_name: str, examples: list, provider="openai"):
    ...
    for ex in examples:
        ...
        try:
            ...
            results.append({
                "baseline": baseline_name,
                "answer": f"Error-fallback answer",
                "contexts": [],
                "ground_truth": ex.ground_truth,
                "domain": ex.metadata["domain"]
            })
    ...
    if not os.getenv("OPENAI_API_KEY") and not os.getenv("GEMINI_API_KEY"):
        ...
        async def mock_generate(self, prompt, **kwargs):
            ...
            if "decomposition" in p_lower or "plan" in p_lower:
                return json.dumps({"sub_queries": [ex.question for ex in examples if ex.question in prompt] or ["General Query"]})

            # 5. Navigation logic (expects JSON)
            import re
            node_ids = re.findall(r"\[([a-f0-9\-]+)\]", prompt)
            chosen = node_ids[0] if node_ids else "mock-id-1"
            return json.dumps({"chosen_id": chosen, "fallback_id": None, "reason": "Relevant node found"})
...
async def main(subset_size: int = 2, provider: str = "gemini"):
    print(f"🚀 Starting Research Benchmark Suite (n={subset_size} per dataset, provider={provider})")
    ...
    baselines = ["LangChain", "LlamaIndex", "Apex-Ablation", "Apex-Full"]
    final_results = []
    ...
    for name in ["LangChain", "LlamaIndex", "ApexRAG-Ablation", "ApexRAG-Full"]:
        ...
        cov = sub.apply(lambda x: 1.0 if str(x["ground_truth"]).lower() in str(x["answer"]).lower() else 0.0, axis=1).mean()
        ...
        if name == "ApexRAG-Full":
             ...
             recall = legal_sub.apply(lambda x: ApexMetrics.contradiction_recall(x["raw_answer"], x["ground_truth"]), axis=1).dropna().mean()
