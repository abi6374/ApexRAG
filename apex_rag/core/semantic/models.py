...
class SemanticModel(BaseModel):
    ...
    detailed_summary: str | None = Field(
        None, description="A longer summary for highly complex sections."
    )
    keywords: list[str] = Field(default_factory=list, description="Extracted semantic keywords.")
    intent_tags: list[str] = Field(
        default_factory=list, description="Tags like 'factual', 'procedural', 'financial'."
    )
    entities: list[str] = Field(
        default_factory=list, description="Named entities found in the text."
    )
    domain_tags: list[str] = Field(
        default_factory=list, description="Domain-specific tags (e.g., 'Q3', 'Revenue')."
    )
