...
class TemporalReasoningAgent:
    ...
    async def solve_temporal_query(self, query: str, doc_id: str) -> dict[str, Any]:
        ...
        if (
            "change" in query.lower() or "compare" in query.lower() or "diff" in query.lower()
        ) and len(dates) >= 2:
            ...
            for key in set(state1.keys()).union(state2.keys()):
                ...
                if isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
                    metric_comparisons[key] = self.change_analyzer.compare_metrics(
        ...
        if dates:
            ...
            return {
                ...
                "result": {"metrics": metrics, "content": doc_text},
        ...
        latest_metrics = await self.reconstructor.reconstruct_metrics(
            doc_id, datetime.now(timezone.utc)
        )
        return {
            "mode": "LATEST",
            "result": {"metrics": latest_metrics, "nodes_count": len(latest_nodes)},
